...
def generate_ajax(request):
    ...
    if request.method == 'POST':
        ...
        if prompt and not is_generating:
            is_generating = True
            generation_progress = 10  # Только начальный прогресс
            ...
            try:
                ...
                try:
                    ...
                    if not created:
                        ...
                        prompt_obj.translated_prompt = english_prompt  # Обновляем перевод на случай изменений
                        prompt_obj.save()
                        print(f"[SUCCESS] Промпт обновлен, использован {prompt_obj.use_count} раз (AJAX)")
                    else:
                        print("[SUCCESS] Новый промпт сохранен в историю (AJAX)")
